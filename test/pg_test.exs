defmodule PGTest do
  ...
  test "inserting data with postgrex", %{data: flares} do
  ...
  def transform_result(result) do
    atomized = for column <- result.columns, do: String.to_atom(column)
    for [id, class, scale, date] <- result.rows, do: List.zip([atomized, [id, String.to_atom(class), scale, date]]) |> Enum.into(%{})
  end
